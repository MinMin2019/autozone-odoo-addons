...
| `custom_branch_consumption_ui` | ใบเบิกใช้วัสดุจอเดียว (MCyyyymmNNNNN) ครอบ operation type CONS: เลือกสาขา (เดาจากคลังของผู้ใช้) + สินค้า/จำนวน มีคอลัมน์ "คงเหลือที่สาขา" เตือนสีแดงเมื่อเบิกเกิน กด "บันทึกเบิก" ครั้งเดียว → สร้าง+validate ใบ CONS ตัดสต็อก ลงบัญชีค่าใช้จ่าย+analytic สาขา, ย้อนวันที่ได้ (ผ่าน custom_stock_backdate), ปุ่ม "เบิกคืน" เปิด Return ของใบ CONS และหักยอดสุทธิ, record rule ขอบเขตคลังแบบเดียวกับ branch_transfer — เมนู Inventory > Operations > เบิกใช้วัสดุ / + เบิกใช้วัสดุ ใบใหม่ |
